...
bool convert_image_(uint8_t *src, uint16_t width, uint16_t height, pixformat_t format, uint8_t quality, jpge::output_stream *dst_stream)
{
    int num_channels = 3;
    ...
    for (int i = 0; i < height; i++) {
        convert_line_format_(src, format, line, width, num_channels, i);
...
bool fmt2jpg_(uint8_t *src, size_t src_len, uint16_t width, uint16_t height, pixformat_t format, uint8_t quality, uint8_t ** out, size_t * out_len)
{
    ...
    if(!convert_image_(src, width, height, format, quality, &dst_stream)) {
        free(jpg_buf);
        return false;
    }
